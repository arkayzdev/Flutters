        ...
        for($i=0; $i<6; $i++){
          ...
          for($i=0; $i<6; $i++){ 
            ...
            $q = 'SELECT * FROM SESSION WHERE id_session IN (SELECT id_session FROM TAKE_PLACE WHERE id_movie = :id_movie) AND seance_date = :seance_date ORDER BY start_time ASC;';
              ...
              if(count($result) != 0){
                  echo '<button style=" box-shadow:  rgba(148, 15, 32, 0.45) 0px 5px 15px; transition: 0.2s; " onclick="calendar_button_trigger(this.value,' . $_GET['id'] . ')" value="' . $calendar[$i] . '" class="calendar_button"><p id="' . $calendar[$i] . '">' . strtoupper(strftime("%a %d %b",strtotime($calendar[$i]))) . '</p></button>';

                } else {
                    echo '<button onclick="calendar_button_trigger(this.value,' . $_GET['id'] . ')" value="' . $calendar[$i] . '" class="calendar_button"><p id="' . $calendar[$i] . '">' . strtoupper(strftime("%a %d %b",strtotime($calendar[$i]))) . '</p></button>';
                }
